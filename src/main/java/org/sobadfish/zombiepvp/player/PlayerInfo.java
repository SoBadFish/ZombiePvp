package org.sobadfish.zombiepvp.player;
...
import org.sobadfish.zombiepvp.event.PlayerGameDeathEvent;
import org.sobadfish.zombiepvp.manager.FunctionManager;
import org.sobadfish.zombiepvp.manager.TotalManager;
import org.sobadfish.zombiepvp.player.message.ScoreBoardMessage;
...
public class PlayerInfo {
    ...
    public void spawn() {
        ...
        if(getTeamInfo() != null){
            sendMessage(getTeamInfo().getTeamConfig().getTeamConfig().getSpawnMessage());
        }
...
//                " 存活人数: &a "+teamInfo.getLivePlayer().size() +" &7/&a "+teamInfo.getTeamPlayers().size()
            ...
            lore.add("       ");
            lore.add(TotalManager.getLanguage().getLanguage("scoreboard-line-point","积分: &a[1]",
                    getData(PlayerData.DataType.COUNT)+""));
        ...
        if (!isWatch()) {
            if (getPlayer().getInventory().getItemInHand().getId() == 345) {
                ...
                for (PlayerInfo info : gameRoom.getLivePlayers()) {
                    if (info.equals(this) || info.getTeamInfo() == null || (info.getTeamInfo() != null
                            && info.getTeamInfo().equals(teamInfo))) {
                        continue;
                    }
        ...
        if(event != null) {
            ...
            if (event.getCause() == EntityDamageEvent.DamageCause.VOID) {
                if(damageByInfo != null){
                    ...
                    addData(PlayerData.DataType.COUNT,teamInfo.getTeamConfig().getTeamConfig().getDeathPoint());
                }
